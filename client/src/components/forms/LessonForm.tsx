...
import { getSubjectTeachers, getRooms,addLesson,updateLesson } from "@/services/api";
...
const LessonForm = ({ type, data }: { type: "create" | "update"; data?: any }) => {
    ...
    useEffect(() => {
        ...
        const fetchRooms = async () => {
            try {
                const firstPage = await getRooms(1, 10);
                const totalPages = firstPage.meta?.last_page || 1;
                const allRoomsPages = await Promise.all(
                    Array.from({ length: totalPages }, (_, i) => getRooms(i + 1, 10))
                );
