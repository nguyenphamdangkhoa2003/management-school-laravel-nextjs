...
const StudentListPage = () => {
    ...
    useEffect(() => {
        const fetchStudents = async () => {
            try {
                const data = await getAllStudentByLessonId(id, currentPage, 10);
    ...
    const renderRow = (item: Student) => (
        ...
        >
            <td className="flex items-center gap-4 p-4">
                <Image
                    src={item?.student.img ? `${process.env.NEXT_PUBLIC_API_URL}/${item.student.img}` : "/avatar.png"}
                    alt="Sinh viên"
                    width={40}
                    height={40}
            ...
            <td className="hidden md:table-cell">{item.grade?.student.level || ""}</td>
